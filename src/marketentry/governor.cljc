(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Bangladesh-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Bangladesh-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-bd-entity?
                                       true` (near-universal for Bangladesh
                                       ...
                                       Grounded in Bangladeshi legal entity
                                       registration typically required for e-GP
                                       awards (contracting authorities / CPTU).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(TIN/統一資格/e-GP/CPTU登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- bd-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-bd-entity? true`, INDEPENDENTLY verify
  `:has-bd-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-bd-entity?` ground truth (most Bangladesh public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-bd-entity? e))
                 (not (true? (:has-bd-entity? e))))
        [{:rule :bd-entity-missing
          :detail (str subject " はバングラデシュ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (bd-entity-missing-violations request st)
