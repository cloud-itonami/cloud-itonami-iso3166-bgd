...
2. Register the client's intake: business type, target public function,
   prior filing history in Bangladesh if any.
3. Run the advisor in read-only mode against e-GP (Electronic Government Procurement), Bangladesh's system managed by the Central Procurement Technical Unit (CPTU).
...
- named referral relationship with Bangladeshi-licensed counsel or a
...
a working referral relationship with Bangladeshi-licensed counsel or a
registered agent for whatever licensed representation the law of
Bangladesh requires for actual public-procurement filings.
