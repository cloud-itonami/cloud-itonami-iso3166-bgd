# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Bangladesh

## Classification

- Repository: `cloud-itonami-iso3166-bgd`
- ISO 3166: `BGD` (Bangladesh)
...
- Social impact: [:domestic-preference-margin-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Bangladeshi
  public contract
...
- registration walkthrough for e-GP (Electronic Government Procurement), Bangladesh's system managed by the Central Procurement Technical Unit (CPTU)
- business/tax registration checklist: a company registration with the Registrar of Joint Stock Companies and Firms (RJSC); a Tax Identification Number (TIN) and VAT registration with the National Board of Revenue (NBR)
- local-content / preferential-procurement navigation: no blanket local-content quota, but certain CPTU tenders apply domestic-preference margins for locally-manufactured goods
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Bangladeshi-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Bangladesh).
